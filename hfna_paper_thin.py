...
Variant: repositions range to follow price on drift>2 (REPO_COST each).
...
Logs: hfna_paper_repo.jsonl ; state: hfna_paper_repo_state.json
"""
...
STATE = os.path.join(MON, "hfna_paper_thin_state.json")
LOG = os.path.join(MON, "hfna_paper_thin.jsonl")
TXFLOW = os.path.join(MON, "txflow.jsonl")
MIN_SWAPS_5M = 75        # entry precondition from txflow_correlation sweep
TXFLOW_MAX_AGE = 150.0   # s — stale tape = no gate pass
...
def latest_swaps5():
    """pool -> (t, swaps_5m) from the newest txflow row (claim-immune meter)."""
    out = {}
    try:
...
def main():
    ...
    if pos:
        ...
        for r in cur:
            ...
            if dpf > 0 and pos["low"] <= ab <= pos["high"]:
                ...
                share = SIZE / (r["liq_active"] / 1e9 + SIZE) if r["liq_active"] > 0 else 1.0
            ...
            if abs(ab - pos["anchor"]) > DRIFT:
                ...
                pos["repo_cost"] = pos.get("repo_cost", 0.0) + REPO_COST
            ...
            if ratio < 1.2:
                ...
                break
            # refill exit: for thin entries (trough ~0) require a REAL refill
            # (>2 SOL at active bin) so dust liquidity doesn't insta-kill the trade
            if r["liq_active"] > max(pos["trough"] * 1.5, 2e9):
        ...
        if pos.get("exit_reason"):
            net = pos["fees"] - FIXED_COST - pos.get("markout", 0) - pos.get("repo_cost", 0)
            st["bankroll"] += net
            st["trades"] += 1
            ...
            with open(LOG, "a") as f:
                f.write(json.dumps({
                    ...
                    "net": net, "reason": pos["exit_reason"], "repos": pos.get("repos", 0),
                    "elev": pos["elev"], "bankroll": st["bankroll"],
                    "mult": lp_mult(p), "entry_swaps5": pos.get("entry_swaps5"),
                }) + "\n")
    ...
    if not st["pos"]:
        ...
        for r in rows:
            ...
            if p in deny:
                continue
            # tx-flow precondition (arm 5): repo follow + confirmed tape flow
            tf = sw5.get(p)
            ...
            # vacuum check — with thin-pool bypass: when active-bin liquidity
            # is ~0 (permanently thin fountains like 8GhQ), a vacuum can never
            # form; the tx5 tape gate (above) is the entry proof instead, and
            # share ~= 1 makes these the highest-capture venues we can enter.
            THIN_LIQ = 1e9  # lamports (1 SOL) at the active bin
            thin = r["liq_active"] < THIN_LIQ
            if not thin:
            ...
            st["pos"] = {
                ...
                "anchor": ab, "repo_cost": 0.0, "repos": 0,
                "last_t": r["t"], "last_pf": r["prot_fee_y"],
                "trough": r["liq_active"], "entry_swaps5": tf[1], "thin": thin,
            }
            st["seen"][p] = time.time()
            print(f"[enter] {p[:8]} HFNA thin Y-only {SIZE} SOL bins [{ab-2},{ab}] elev={ratio:.1f}x tx5={tf[1]} thin={thin}")
            break
