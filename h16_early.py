#!/usr/bin/env python3
"""H16-EARLY shadow forward test (§415) — zero-risk OOS evidence for the mom+dd combo.
...
All results appended to h16_early.jsonl. No orders, no wallet touches.
"""
import json, time
...
STATE = MON / "h16_early_state.json"
LOG = MON / "h16_early.jsonl"
    ...
    with LOG.open("a") as lg:
        ...
        for mint in list(st["pending"]):
            ...
            if not ts:
                ...
                if age > MAX_PENDING_AGE:
                    st["pending"].pop(mint)
                    lg.write(json.dumps({"action": "h16e_drop", "mint": mint,
            ...
            if len(pre) < 3 or not m0:
                st["pending"].pop(mint)
                lg.write(json.dumps({"action": "h16e_thin", "mint": mint,
            ...
            if mom >= MOM_Q67 and dd >= DD_Q67 and bf >= BF_Q67:
                ...
                lg.write(json.dumps({"action": "h16e_open", "mint": mint,
                                     "t": pre[-1][0], "scored_at": now,
                                     "entry_mcap": pre[-1][3], "mom": round(mom, 3),
                                     "dd": round(dd, 3), "bf": round(bf, 3),
                                     "seed": p["seed"]}) + "\n")
            else:
                lg.write(json.dumps({"action": "h16e_reject", "mint": mint, "t": now,
        ...
        for mint in list(st["open"]):
            ...
            if close:
                ...
                st["open"].pop(mint)
                lg.write(json.dumps({"action": "h16e_close", "mint": mint, "t": ct,
