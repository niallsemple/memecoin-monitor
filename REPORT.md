...
## §250 — Closes #55-56: Nkb5 +0.00092, X11c abort15 +0.00058 — two more small greens
- Nkb5DVdr (0.1022, deployer 4ZqzgAFC first-seen): abort15 at 18.4min, verified meta.err None gross +0.10281.
...
## §287 — Entry-speed profile: discovery is 1s; the 54s is gate evidence (4 Sep 2026, ~20:15 UTC)
...
## §307 — Trade #13 closed (n=13)

- **AhiWggtX**: conv_override 0.05 SOL → **+0.00092 SOL (+1.8%)**, held 15.5 min, exit `abort15` (scratch-green class).
- Live book: **13 closed — 12 green / 1 scratch red**, cumulative **+0.01301 SOL**, expectancy ~+2.0%/trip, zero wipeouts.

## §308 — Trade #14 closed (n=14)

- **b9jdJeHu**: conv_override 0.05 SOL → **+0.00027 SOL (+0.5%)**, held 15.1 min, exit `abort15` (scratch-green class).
- Live book: **14 closed — 13 green / 1 scratch red**, cumulative **+0.01328 SOL**, zero wipeouts.

## §309 — Trade #15 closed (n=15)

- **thaNgAcv**: conv_override 0.05 SOL → **+0.00204 SOL (+4.1%)**, held 15.1 min, exit `abort15` — best abort15 close of the cohort.
- Live book: **15 closed — 14 green / 1 scratch red**, cumulative **+0.01532 SOL**, zero wipeouts.

## §310 — Trade #16 closed (n=16)

- **fSDKAEni**: conv_override 0.05 SOL → **+0.00061 SOL (+1.2%)**, held 15.4 min, exit `abort15` (scratch-green class).
- Live book: **16 closed — 15 green / 1 scratch red**, cumulative **+0.01593 SOL**, zero wipeouts.

## §311 — Trade #17 closed (n=17)

- **kQyYsf43**: conv_override 0.05 SOL → **+0.00104 SOL (+2.1%)**, held 15.5 min, exit `abort15` (scratch-green class).
- Live book: **17 closed — 16 green / 1 scratch red**, cumulative **+0.01697 SOL**, zero wipeouts.

## §312 — Trade #18 closed (n=18)
...
## §318 — First 0.10 SOL trade closed (n=23, scale-up 1/10)
...
## §320 — Trade #25 closed (n=25, scale-up 3/10)

- **J8weKmr9**: conv_override 0.10 SOL → **+0.00108 SOL (+1.1%)**, held ~16 min, exit `abort15` (scratch-green class).
- Footprint: buy PI **0.12%**, sell PI **0.43%** — buy side well under the 1.2% threshold; sell PI highest of cohort so far but small in absolute terms.
- Scale-up cohort: **3/10 done**, cumulative **+0.00540 SOL** (+4.5%, −0.14%, +1.1%); buy PIs 0.21/0.16/0.12%.
- Live book: **25 closed — 23 green / 2 scratch-red**, cumulative **+0.02639 SOL**, zero wipeouts.

## §321 — Trade #26 closed (n=26, scale-up 4/10)

- **gz9VFREJ**: conv_override 0.10 SOL → **+0.00083 SOL (+0.8%)**, held ~16 min, exit `abort15` (scratch-green class).
...
## §366 — Kamino refresh bundle built; remaining-account head fix; the twilight-zone discovery (2026-09-05 ~20:50 BST)

**Build 1 — refresh_reserve bundle:** kamino_lending IDL fetched (KLend2g3cP87fffoy8q1mQqGKjrxjC8boSyAYavgmjD; refresh_reserve disc [2,218,138,235,79,201,25,102]; accounts: reserve(w), lending_market, pyth, swb, swb_twap, scope). Oracle slots are filled from the RESERVE's own token_info config — offsets computed by walking the full IDL type tree: scope price_feed @5112, swb agg @5160 (+twap @5192), pyth @5224 (on-chain, incl. 8B disc); all-default slots take the KLend program id as placeholder. The USDC reserve prices via **Scope** (3t4JZcue…), not Pyth — first attempt with the marginfi bank's oracle failed InvalidPythPriceAccount 6054, which is what exposed this. liq_sim now prepends a refresh ix for every Kamino bank either account touches.

**Build 2 — remaining-account head fix (the real schema):** liquidate.rs:260-272 consumes `get_remaining_accounts_per_bank(asset)-1` then the same for liab from the HEAD of remaining accounts. For a Kamino asset bank that's oracle+reserve (2 accounts), not 1. Our old layout [asset_oracle, liab_oracle, ...] shifted everything by one — WrongOracleAccountKeys 6052. Fixed: head = bank_remaining(asset)[1:] + bank_remaining(liab)[1:].

**Result:** full pipeline sim runs clean through refresh + pricing + health gates on GPkitqFXLPAM. Program confirms pre_health **−7.48** (scanner: −0.021 ratio, $337.87 vs $345.38 — agreement again).

**Discovery — the twilight zone:** every seize size fails WorseHealthPostLiquidation 6072. Per-dollar health delta = −aw_asset + (1−fees)×lw_liab = −1.0 + 0.95×1.05 = **−0.0025** — always negative regardless of size. Accounts whose collateral/liability weight pairs all violate `aw < (1−fees)×lw` are underwater but structurally un-liquidatable via the standard ix; they wait for receivership. (Larger sizes also hit OperationRepayOnly 6022 — the BTC banks are repay-only.) GPkitqFXLPAM is a phantom: visible, priced, forever out of reach.

**Scanner upgrade:** liquidation fee offsets located empirically (liquidator_fee u32 @1544, insurance_fee u32 @1548, anchored via integration_acc_1 @1560; 0 = default 2.5%). health_scan now tags every health<0 account with `feasible`: whether ANY asset/liab slot pair satisfies the inequality. Latest scan: 20 liquidatable-now, 11 pair-feasible (all currently $0-collateral bad debt), 9 twilight. Future volatility candidates will be pre-filtered for actual seizability — no more drilling phantoms.
...
## §368 — Drift + JupLend multipliers: scanner coverage now 437/437 priced banks (2026-09-05 ~21:50 BST)

**Build:** last two unpriced classes closed. Drift (9 banks, setup 9): mult = cumulative_deposit_interest u128 / 1e10 from MinimalSpotMarket (drift-mocks state.rs; disc [100,177,8,107,168,65,65,39]; field @464 on-chain). JupLend (7 banks, setup 15): mult = token_exchange_price u64 / 1e12 from Lending (juplend-mocks state.rs; repr(C, packed); disc [135,199,82,16,249,131,182,241]; field @115 on-chain). Both use oracle_keys[1] for the integration account (confirmed in price.rs load_drift_spot_market/load_juplend_lending); Pyth feeds at keys[0] added to the price map (SwbPull variants 10/16 handled too, none on-chain).
...
## §370 — Full §337 flash-liquidation recipe assembled + structurally sim-verified

**Built this session (all in liq_flash.py unless noted):**
- `build_v0_tx(payer, ixs, alt_pks)` — v0 message compiler with address lookup tables. Two bugs found and fixed en route: (1) a 30-char system-program constant in my test harness produced a 30-byte key and shifted every downstream offset (module constants verified 32B); (2) program ids must stay STATIC — a compiled ix whose program_id_index resolves through a lookup table fails runtime sanitize ("Transaction failed to sanitize accounts offsets correctly"). Compiler proven via minimal v0+ALT transfer sim err:None.
- `liq_alt.py` — created OUR OWN 5 address lookup tables on-chain holding the main-group marginfi key universe: 1099 keys (204 main-group banks × bank+oracle keys+3 vault PDAs, Kamino refresh bundles incl. reserve/lending-market/oracle slots, token/ATA/system/CB/sysvar/SWB programs, our mfi acct, group). Cost 0.0593 SOL. Resumable (liq_alts.json fingerprint); fresh slot per table (seed slots go stale after ~150 slots); 3s retry on preflight races (RPC node lag after create → spurious InvalidAccountOwner).
- Jupiter leg via `POST /swap/v1/swap-instructions` — returns raw instruction objects spliced into our own v0 message + its route's ALTs (fail-closed if any ix wants a foreign signer).
- `build_recipe(tee, asset_bank, liab_bank, amt, swap_in, payer)` = compute budget → idempotent ATA creates → start_flashloan(end_index) → [Kamino/SWB refreshes] → liquidate → withdraw_all(seized) → Jupiter swap asset→liab → repay_all → end_flashloan(obs). Final message: 15 ixs, **937 bytes** (was 1253–1615B over limit before our ALTs).

**Structural sim results (processed, sigVerify off):** ATA creates run → start_flashloan ok → liquidate reached and executes marginfi domain logic. Against a real bad-debt account (JBo7t6Cq): 6002 LendingAccountBalanceNotFound (no SOL collateral to seize — correct). Against own account: AccountBorrowFailed (self-liquidation artifact). Full green sim is impossible without a live underwater-with-collateral candidate — fail-closed behavior is correct by design.
...
## §389 — Gate hole closed: farm denylist now enforced at EVERY entry chokepoint (2026-09-06 02:35 UTC)

Rug #3 (oZkqKQgk, −0.0687 SOL): the farm coin §385 blocked at birth (00:50 UTC, seed 85.005) re-entered 83 min later through the GRADUATED s60nm5fr hook path — which had no seed gate — and drained 4.8 min after we bought (1,628 SOL single sell, mcap 152k→2). My emergency sell landed 7s post-drain; the exit engine's own abort3_red writeoff fired within the same second. Lesson: birth-window gating is necessary but not sufficient; the farm's pools stay dangerous for their whole life.

Fix: persistent farm_denylist.json (backfilled: 1,886 mints all-time with create seeds ≥50 SOL) checked at the two universal chokepoints — live_trader.buy() and curve_buy() — so every current and future entry path refuses farm mints. The §385 birth gate now appends each new block to the denylist automatically. Smoke-tested: farm mint refused, normal mint passes. live_trader loads fresh each pass; tracker deployed. §376-era sample: 4 opens / 4 closed, net −0.1633 (1 win, 1 flat, 2 farm rugs).
...
## §452 — Stale-guard validation: the 5-min gap is a FILTER, not a leak (2026-09-07 ~05:00 UTC)
Triggered by D3kV8bi6 (seed 6.91) stale-skip. Root cause of stale skips: tracker runs 15-min windows on a ~20-min scheduler grid → ~5-min off-gap between runs; gap-born candidates score at next window start with 294-304s lag > STALE_OPEN_S=150. Extended blocked_shadow_e2.py to replay bridge_skip_stale mints (entry = first tape print >= bridge decision ts = the actual late price we would have paid). Result, all 3 stale skips of the era: CeDFmAkP peak=1.000 abort15 −0.00078; 8iUM4VAT peak=1.018 abort15 −0.00441; D3kV8bi6 peak=1.001 abort3_red −0.00763. TOTAL −0.01282 SOL AVOIDED. None of the late entries EVER traded >2% above the would-be entry — 60s-window momentum is dead by minute 5. CONCLUSION: entry timing IS the edge; the stale guard earns. No fix warranted — do NOT loosen STALE_OPEN_S and do NOT build an always-on scorer to catch gap births. Blocked cohort now n=22, gates earn 0.08393 SOL.

## §453 — First organic post-fingerprint live exit: 3UEaxQUP −0.00757 (2026-09-07 05:36 UTC)
First clean-stack organic entry: seed 6.422 (NOT farm fingerprint), mom 1.009 / dd 0.991 / bf 0.80, entry_mcap 41.8, open_age 18.7s (fresh, in-window). bridge_buy 0.02 SOL → 402.3B tokens, sig 3jXToBhR…, entry 05:34:22. Price dumped immediately: peak 1.011, trough 0.747 within ~70s → panic exit (r<0.80) at 05:35:35, recovered 0.01243, PnL −0.00757 SOL (−37.9% of stake). Shadow barrier model recorded note_close 0.92 "loss" 59s post-open — consistent. Post-gate organic sample: n=1, −0.00757. Gates all functioned (fresh entry, fingerprint passed, serial passed); the loss is the trade itself, not a stack failure. Coin kept bleeding after our exit (tape 27.98 mcap = 0.67x entry at +5min) — panic cut was correct ex-post.

## §454 — Second organic post-gate exit: 7xyjLwR5 −0.00966 (2026-09-07 06:05 UTC) — WARNING PATTERN
Seed 7.408 (organic), mom 1.031 / dd 0.970 / bf 0.714, entry_mcap 45.1, fresh (12.1s lag). Bought 0.02 → 373.4B tokens 05:59:42. Price NEVER traded above our fill (peak_mult 1.000), slid to trough 0.606 → abort3_red at +5.6min sold all at ~0.52x avg → recovered 0.01034, PnL −0.00966 (−48.3% of stake). Post-gate organic sample n=2: both losses (−0.00757, −0.00966), and BOTH entries had peak ≤1.011 — our fill was systematically the top tick. Hypothesis to test at n≥4: 60s mom/dd/bf gates now select coins whose pump ENDS at the 60s mark (window buyers = exit liquidity for snipers); the ~12-19s execution lag puts our fill at/above the qualifying print. Possible refinement if pattern persists: execution-time confirmation (skip if current mcap < 0.97× the qualifying print, i.e. momentum already gone at fill). Do NOT change gates on n=2.

## §455 — Confirmation-entry shadow test (2026-09-07 ~06:55 UTC)
After 3/3 live organic losses all printed peak=1.0 (fill = top), tested a selection-side variant on the full seed>=4 h16 cohort (n=13, tape-replayed through the exact e2 exit stack): CONFIRMATION ENTRY = wait until +120s post-birth, buy only if price >= the 60s qualifying print, else skip. FULL cohort: baseline −0.5465 ret (−0.01093 SOL) → confirmation −0.2778 (−0.00556), 6 skips. ORGANIC-ONLY (farm seeds excluded, n=8): baseline −0.0215 (−0.00043 SOL, ~breakeven) → confirmation +0.2161 (+0.00432 SOL). Caveats: (a) confirmation skipped two winners (8quDSNDD +0.16, EW4KWShw +0.16); (b) on the 3 live losses it would skip 7xyjLwR5 (save −0.40), WORSEN 3UEaxQUP (−0.27→−0.32), flip B3Eqo3Lc to ~0 — mixed, not a clean save; (c) n=8 organic is small. DECISION: do NOT rewire the live bridge yet. Keep accumulating live exits at 0.02; rerun this test as the cohort grows. If live entries #4-5 also print peak≈1.0, wire confirmation into the bridge (delay buy to +120s, require last mcap >= qualifying print).

## §456 — Concurrency-slot bug found + fixed (2026-09-07 ~07:15 UTC)
E59sCeKG (seed 4.94) was blocked by max_concurrent while NO live position was open. Root cause: the bridge cleared open_mints only on the shadow h16e2_close (barrier 1.2/0.92 or 1h timeout), so after the LIVE exit stack closed B3Eqo3Lc at 06:50, the slot stayed occupied until the shadow close — up to ~1h of false blocking after every trade. Fix: e2_live_bridge.run_pass now reconciles open_mints against live_positions.json every pass (drop any mint whose live position is closed). Verified live: stale B3Eqo3Lc slot cleared, open=0. The blocked candidate itself replayed to ~breakeven (+0.00006 SOL) — no economic harm this time, but the bug could have blocked a runner. Blocked cohort n=24, gates earn 0.08466.

## §457 — Second farm fingerprint burned us live; SEED_DENY +1 (2026-09-07 ~08:05 UTC)
srxbT3rU (seed 9.868646486) passed all gates at 07:59, bought 0.02 → abort3_red +4.9min, trough 0.574, PnL −0.01044 SOL. Seed audit: EXACT seed 9.868646486 on 6 launches (4 stale-drops 09-06 15:04-22:04, BUsMgA4f open 09-07 07:19 [stale-skipped; replay −0.0395], srxbT3rU 07:59 [live loss]). Same §450 pattern: creator rotation, fixed seed. Added 9.868646486 to SEED_DENY (now [5.264197529, 9.868646486]); tracker fast-loop reloads the module each 15s pass so the gate is live immediately. Bookkeeping: this trade is FARM, not organic — organic post-gate sample stays n=3 (−0.0208). Both live farm burns (−0.00909 §450, −0.01044 §457) now covered by fingerprints; each new fingerprint costs at most one 0.02 trade before denial.

### §458 — Kamino radar breakthrough: crank-tail replaces gPA (Helius blocks KLend program scans)
- Helius rejects ALL `getProgramAccounts` against the KLend program (`WrongSize` regardless of slice/filters — program-level block, ~millions of accounts). Root-caught a second bug first: the KLend program id in my notes had 2 phantom trailing chars (`...gmjDhe`, 45 chars/33 bytes). Real id from the local full IDL: `KLend2g3cP87fffoy8q1mQqGKjrxjC8boSyAYavgmjD` (32 bytes).
- New file `kamino_tail.py`: tails `getSignaturesForAddress(KLEND)`, parses txs for `refresh_obligation` ixs (disc [33,132,147,228,151,192,72,89], account[1]=obligation), batch-fetches the refreshed obligations, decodes stored health fields — `borrow_factor_adjusted_debt_value_sf` @2208 vs `unhealthy_borrow_value_sf` @2256 (u128 /2^60, layout verified against `refs/kamino_lending_full.json`: ObligationCollateral=136B ×8, ObligationLiquidity=200B ×5, BigFractionBytes=48B).
- First pass: 100 sigs / 157s → 4 refreshed obligations, 3 with debt, all healthy (largest: $134k adj-debt vs $160.7k trigger, margin 17%, slot_age 242 ≈ fresh). Decode sanity-checks out (ordering consistent, freshness matches the crank tx we saw).
- Why this design is RIGHT not just expedient: cranks only refresh obligations near the line — the tail is a pre-filtered risk feed. Calm-market rate ≈ 1.5 refreshes/min; vol spikes will flood it exactly when EV appears.
- Open gaps: (1) local IDL is a mock subset — no `liquidate_obligation_and_redeem_reserve_collateral` ix; must source real KLend IDL for the fire path. (2) Tail only sees refreshed obligations; need continuous runner + near-line margin threshold (e.g. <5%) to build the watch set. (3) Staleness check via slot_age works (saw 242 vs 38180).

### §459 — Kamino radar is LIVE inside the tracker (kamino_loop thread, 240s cadence)
- `kamino_tail.run_pass()` added (harvest 60 sigs → health decode → refresh `kamino_tail_latest.json`; appends to `kamino_alerts.jsonl` when margin < 0 = LIQUIDATABLE or < 5% of adj-debt = near_line). Never raises (daemon-thread safe).
- `tracker_live.py` gained a `kamino_loop` daemon thread alongside shock_loop/e2_fast_loop, same importlib-reload pattern so kamino_tail.py edits go live without sync. Synced workspace → automation assets (md5 117d334d… match); next scheduled automation window picks it up.
- Standalone run_pass test: 4 refreshed obligations decoded, no near-line hits (calm tape). Alert tape will light up during vol spikes — same asymmetry as marginfi shock path but on Solana's biggest lending book.
- Still open for the Kamino fire path: real KLend liquidate ix layout (local IDL is a mock subset — fetch full IDL from Kamino-Finance/klend repo when an alert actually fires; building it blind wastes nothing since it can't be tested without a live liquidatable account anyway).

### §460 — Kamino fire-path spec locked: production IDL pulled, layouts verified, discs computed
- Full production KLend IDL (66 ixs) saved to `refs/klend_idl_full.json` (from @kamino-finance/klend-sdk@11.0.2). GitHub repo has no IDL; on-chain IDL PDA empty.
- **Layout risk closed**: production ObligationLiquidity grew 2 fields (fixedTermBorrowRolloverConfig 16B, borrowedAmountAtExpiration 8B) but padding2 shrank 56→32B — total still 200B, so ALL radar offsets (adj_debt@2208, unhealthy@2256) are production-valid. ObligationCollateral identical (136B).
- Liquidate ix (flat V1, easiest to hand-build): disc [177,71,154,188,226,133,74,55], 20 accounts (liquidator signer, obligation, lendingMarket+authority, repayReserve trio, withdrawReserve quad, fee receiver, 3 user token accounts, 3 token programs, ixs sysvar), args (liquidityAmount u64, minAcceptableReceived u64, maxAllowedLtvOverridePct u64). V2 disc [162,161,35,143,30,187,185,103] uses packed LiquidationAccounts — defer to SDK if ever needed.
- Native flash loans confirmed: flashBorrowReserveLiquidity disc [135,231,52,167,7,52,212,193] / flashRepayReserveLiquidity disc [185,117,0,203,96,245,180,186] (borrowInstructionIndex arg) → zero-capital fire path shape: flashBorrow(repay asset) → liquidateV1 → swap collateral→repay (Jupiter) → flashRepay, same skeleton as the marginfi hunter.
- Next slice: `kamino_hunt.py` — resolve repay/withdraw reserves from the obligation's borrows[]/deposits[] arrays (borrowReserve/depositReserve pubkeys at known strides), build the 3-ix bundle, dry-run simulation on the first real alert.

### §461 — Kamino fire path: serialization VALIDATED on-chain via simulation
- kamino_hunt.py built: decode_obligation → resolve (biggest borrow/deposit + reserve accounts) → build_liq_ix (V1 flat 20 accounts) → simulate via live_trader.build_legacy_tx + sigVerify=false.
- PDA saga closed: managed python lacks nacl → wrote pure-python ed25519 on-curve check; cross-validated against official @solana/web3.js (exact match on ATA + candidate PDAs). Kamino market authority seed = **b"lma"** (from SDK dist/utils/seeds.js), verified byte-exact against an on-chain reserve vault owner (FsvTiXTU…).
- Live sim progression: (1) ATA missing → AccountNotInitialized on user_source_liquidity = account order correct; (2) with ATA-creation ixs → KLend's check_refresh panics on the 1-byte ATA ix data while scanning the instructions sysvar → **ATAs must be pre-staged in a separate setup tx**, and the fire tx must contain refreshReserve + refreshObligation ixs (discs + account lists extracted from production IDL; refreshReserve needs pyth/swb/scope oracle accounts from the reserve config).
- Final fire recipe: [refreshReserve(repay), refreshReserve(withdraw), refreshObligation, flashBorrow(repay), liquidateV1, jupSwap(seized→repay), flashRepay] — all ≥8B data ixs, sysvar-scan safe.
- Reserve offsets for the fire path (computed from production IDL, total 8624B = 8616+8 ✓): lendingMarket@32, liq.mint@128, liq.supplyVault@160, liq.feeVault@192, liq.decimals@272, col.mint@2560, col.supplyVault@2600, protocolLiquidationFeePct@4871.

### §462 — Kamino fire path pre-staged: 12 ATAs live on-chain, zero setup latency
- Fixed kamino_tail.run_pass staleness: always writes kamino_tail_latest.json ({"ts", rows}) so file age = radar liveness.
- kamino_prestage.py: pulls top-6 main-market reserves (SOL, dSOL, USDC, JITOSOL, cbBTC, JupSOL) and CreateIdempotent-stages our ATAs for each **liquidity mint** (needed as userSourceLiquidity/userDestinationLiquidity) and each **kToken collateral mint** (userDestinationCollateral; kToken mints read from reserve account @2560).
- 4 setup txs landed finalized: 2R77qFVT…, 3j59ctLz… (liq mints), 2MjtPN2m…, 4nba7NYb… (kTokens). Confirmed 12/12 exist. Cost ≈ 12 × 0.00204 SOL rent, recoverable by closing. (First confirm raced finality — getSignatureStatuses showed both txs landed err=None.)
- Kamino front state: radar live (240s), decode production-verified, liquidate ix serialization validated in sim, accounts pre-staged. Remaining: full fire bundle (refreshReserve ×2 + refreshObligation + flashBorrow + liquidate + jup swap + flashRepay) — needs reserve oracle account offsets (pythOracle/swbPrice/scopePrices from reserve config), built when the first alert fires so we test against a real target.

### §463 — KAMINO FIRE PATH VALIDATED: full 6-ix bundle executes through liquidate in sim
- Fixed kamino_tail path bug (relative writes went to automation cwd — now MON-anchored; loop picks it up via importlib reload, no sync).
- Oracle wiring: reserve config offsets — scope@5112, swbAgg@5160, pyth@5224, flashLoanFeeSf@4904. Unused oracle slots take the **KLend program id as placeholder** (ground truth from on-chain refreshReserve tx 2YZQmJ3U…).
- refreshObligation needs remaining accounts = obligation's active deposit reserves then borrow reserves (slot order) — else InvalidAccountInput.
- Flash rules (from klend source flash_ixs.rs): repay arg liquidity_amount must EQUAL borrow amount (program adds the fee itself); borrowInstructionIndex = borrow's own index.
- check_refresh position rules (refresh_ix_utils.rs): refreshes must be IMMEDIATELY before liquidate — [refreshReserve, refreshReserve, refreshObligation, LIQUIDATE]; reserves ordered so withdraw-refresh sits at current−2, repay-refresh at current−3. flashBorrow goes FIRST (index 0). Farms ixs (RefreshFarmsForObligationForReserve) required pre+post liquidate when a reserve has a farm set — handle at fire time.
- Pre-staged the target obligation's kToken + underlying ATAs (txs E5ymTC29…, 3cLa2ifk…). All AccountNotInitialized layers peeled.
- FINAL SIM on healthy obligation 6LTRK3Amx: flashBorrow ✓ refreshReserve ✓ ×2 refreshObligation ✓ **liquidate ✓ (KLend success)** flashRepay ✗ "insufficient funds" — expected (healthy obligation seizes nothing to cover repay+fee; atomic revert protects us). This is the fire-ready signature: everything serializes, resolves, and executes; the bundle only profits when the target is truly underwater.
- Remaining single piece: Jupiter swap leg (seized underlying → repay asset) inserted before flashRepay at fire time via liq_fire's quote pattern. Radar flags → resolve → build → simulate → fire.

### §464 — KAMINO PIPELINE COMPLETE: full bundle incl. Jupiter swap runs end-to-end in sim
- Legacy tx hit the 1232B wire cap (7 ixs, ~60 unique accounts) → moved to v0 + Address Lookup Table. Built our own ALT on-chain: `ENJ79NSgueMjXMraVvGj3xfb8YXTtWwyRHrYdApCSFSL` (create tx 2f2F61Z4… finalized), extended with 31 core keys. Gotchas solved: mistyped ALT program id (25 trailing 1s not 24 — decoded 31 bytes, shorting the tx by exactly the "1 byte" the RPC complained about); CreateLookupTable needs a slot that actually produced a block (walk-down retry); ExtendLookupTable uses bincode (u64 vec length, NOT shortvec).
- My hand-rolled v0 serializer kept failing RPC sanitize ("accounts offsets"); rather than keep debugging blind, moved compile+sign to `fire_compile.js` (official @solana/web3.js, vendored into repo node_modules). Python orchestrates, node compiles. Reference-compiled tx passed sanitize immediately.
...
## 2026-09-08 (cont.) — Minute-zero birth detection + DAMM v2 venue discovery

**The birth-venue finding (09:41 UTC):** DLMM is no longer where memecoin pools are born. Sampling real `created_at` timestamps: 0/80 DLMM pools created in the last 7 days (newest sample pool ~6 weeks old) vs **41/80 DAMM v2 (cp-amm) pools**. DAMM v2 datapi (damm-v2.datapi.meteora.ag) tracks 137,646 pools with full created_at/launchpad/tvl/fee_tvl_ratio fields. The two green rotation positions (LIGER, S3) came via raydium-launchlab-tagged DLMM pools; the current flow is met-dbc graduations → DAMM v2.

**Birth listeners (both live, running every 20min):**
- `dlmm_birth_listener.py`: one getProgramAccounts call (dataSize=904, dataSlice=0) over the DLMM program, diff vs previous run = births. Baseline 156,960 pools. First birth: PUGCOIN-SOL (pump.fun) caught at age 5.3min.
- `damm_v2_birth_listener.py`: same trick on cp-amm program `cpamdpZCGKUy5JxQXB4dcpGPiikHawvSWAd6mEn1sGG`, dataSize=1112. Gotchas found: account size is 1112 not 1113 (base64 estimate off-by-one matched zero accounts); the pool-discriminator memcmp is silently ignored by the RPC (1,458,013 accounts in the size class vs 137k tracked pools — class likely includes position accounts), so each birth is resolved against the REST API and flagged `resolved`. First 20min: 29 births, all resolved, all met-dbc, caught at age 5-17min. Rate ~87/hour.

**Collector DAMM v2 pass:** newborns from damm_v2_births.jsonl (<24h) plus carried young pools get per-pool API snapshots into `damm_v2_fee_snapshots.jsonl` (youngest-first, cap 300/run). First pass captured all 29 newborns at age 7-10min. Bug fix: `age_h=None` when created_at missing (was 0.0, which passes young-pool sim gates; historical contamination confirmed harmless — 5 rows, 0 qualifiers).

**Infrastructure (09:30-10:40 UTC):** EVM watcher runs were TIMING OUT at the 20-min turn cap (EVM steps grew) and dying before the Meteora steps — birth detection silently starved. Fix: Meteora pipeline split into dedicated automation_292018b4 (20-min interval: dlmm births, damm v2 births, fee collector); EVM watcher cap raised to 45min. Also: platform cron-trigger wedge confirmed platform-wide — all schedule-trigger automations dead (board meeting missed 3d, nightly backup 7d, forward-test review 10d, heartbeat 7d). Four goal-critical ones converted to 24h interval triggers anchored at their original London times; heartbeat manually run, Edge Vault widget refreshed (binding delivery confirmed).

**`damm_v2_rotation_sim.py` built:** cp-amm economics — exact full-range IL (2√r/(1+r), no bin wipeout), exact cum_fees-diff accrual, exits: dump_stop (r≤0.5), tvl_drain (<30% of entry TVL), fee_dead (absolute+relative decay), max_hold, rotation to better expected-fee pool. 27-cell grid (entry age 1/3/6h × ftr 0.05/0.10/0.25 × min-tvl 1k/5k/10k). Smoke test on first 29 newborns: n=0 as expected (dust TVL at birth; eligible once pools cross $1k+).
