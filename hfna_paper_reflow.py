...
STATE = os.path.join(MON, "hfna_paper_reflow_state.json")
LOG = os.path.join(MON, "hfna_paper_reflow.jsonl")
...
MAX_REPOS = 2         # churn cap (ported from thin 09-13): 55Ev uncapped follow = -0.005
...
def main():
    ...
    if pos:
        ...
        for r in cur:
            ...
            # 55Ev churned ~10 repos for -0.005 on 2026-09-13 (uncapped follow
            # on a drifting, cold-fee pool); thin's MAX_REPOS=2 caps this leak
    ...
    if not st["pos"]:
        ...
        for r in rows:
            ...
            st["pos"] = {
                ...
                "trough": r["liq_active"], "entry_swaps5": tf[1],
            }
            st["seen"][p] = time.time()
            print(f"[enter] {p[:8]} HFNA reflow Y-only {SIZE} SOL bins [{ab-2},{ab}] elev={ratio:.1f}x tx5={tf[1]}")
            break
