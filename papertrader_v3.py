#!/usr/bin/env python3
"""Paper trader v3: EARLY MOMENTUM, STRICT floor (from v2 live session 2026-08-26).

v2 lesson: the single v2 loss (DMoRRhei, stopped -53%) entered at slope exactly
1.00 — the weakest in-sample bucket (45% win). In-sample: slope >=1.25 -> 60%
win, >=1.5 -> 75% win / +19% median. v3 = v2 with floor raised to 1.25.
...
Entry: Solana, verdict PASS, detected after window start, age >= +5 min,
       current mcap >= 125% of detection mcap (strict momentum floor),
       detect liq >= $20k.
...
Costs: 1.5% per side.
State: paper_v3.json. Log: trades_v3.md.
"""
import json
...
FLOOR = 1.25       # STRICT: current mcap >= 125% of detect mcap
MIN_LIQ = 20000
