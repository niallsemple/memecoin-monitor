...
Position: paper Y-only 0.1 SOL spread over WIDTH=7 bins [ab-6, ab].
Variant: boundary early exit when active bin within 2 of floor.
...
STATE = os.path.join(MON, "hfna_paper_w7_state.json")
LOG = os.path.join(MON, "hfna_paper_w7.jsonl")

SIZE = 0.1            # SOL deployed
WIDTH = 7            # bins in range (w7 variant)
EARLY_BUF = 2        # early exit when ab within N bins of floor
...
def main():
    ...
    if pos:
        ...
        for r in cur:
            ...
            if dpf > 0 and pos["low"] <= ab <= pos["high"]:
                ...
                dep_bin = SIZE / WIDTH
                share = dep_bin / (r["liq_active"] / 1e9 + dep_bin) if r["liq_active"] > 0 else 0
            ...
            # boundary early exit: ab within EARLY_BUF of floor -> out before
            # conversion; partial markout only for bins already crossed
            if pos["low"] <= ab <= pos["low"] + EARLY_BUF:
                crossed = max(0, ab - pos["low"])  # bins below ab are crossed
                if crossed > 0:
                    bs = r.get("bin_step", 100)
                    price_ratio = (1 + bs / 1e4) ** (-crossed)
                    pos["markout"] = SIZE * (crossed / WIDTH) * max(0, 1 - price_ratio)
                pos["exit_reason"] = f"boundary early-exit ab={ab} low={pos['low']}"
                break
    ...
    if not st["pos"]:
        for r in rows:
            ...
            ab = r["active_bin"]
            st["pos"] = {
                "pool": p, "t_entry": r["t"], "low": ab - (WIDTH - 1), "high": ab,
                "fees": 0.0, "markout": 0.0, "elev": ratio,
                "last_t": r["t"], "last_pf": r["prot_fee_y"],
                "trough": r["liq_active"],
            }
            st["seen"][p] = time.time()
            print(f"[enter] {p[:8]} HFNA paper Y-only {SIZE} SOL bins [{ab-(WIDTH-1)},{ab}] elev={ratio:.1f}x")
            break
