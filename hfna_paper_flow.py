...
STATE = os.path.join(MON, "hfna_paper_flow_state.json")
LOG = os.path.join(MON, "hfna_paper_flow.jsonl")
...
def latest_swaps5():
    """pool -> swaps_5m from the newest txflow row (claim-immune flow meter)."""
    out = {}
    try:
    ...
    if not st["pos"]:
        ...
        for r in rows:
            ...
            if p in deny:
                gate_stats["deny"] += 1
                continue
            # tx-flow precondition (arm 4): pool must show >= MIN_SWAPS_5M on
            # the claim-immune swap tape, measured within TXFLOW_MAX_AGE
            tf = sw5.get(p)
            if not tf or time.time() - tf[0] > TXFLOW_MAX_AGE:
                gate_stats["no_tape"] += 1
                continue
            if tf[1] < MIN_SWAPS_5M:
                gate_stats["low_txflow"] += 1
                continue
            ...
            st["seen"][p] = time.time()
            print(f"[enter] {p[:8]} HFNA paper-flow Y-only {SIZE} SOL bins [{ab-2},{ab}] elev={ratio:.1f}x tx5={tf[1]}")
            break

    if not st["pos"] and gate_stats:
        gs_path = os.path.join(MON, "gate_stats_flow.json")
        try:
