#!/usr/bin/env python3
"""Parse KNOTS pool activity from knots_txs_pre.jsonl via reserve-vault balance deltas.
...
Outputs:
  knots_swaps_pre.jsonl        one record per tx that swapped on our pool
  knots_prot_claims_pre.jsonl  protocol-fee claims (reserve -> protocol receiver)
  knots_lp_claims_pre.jsonl    LP fee claims (incl. our own guardian claims)
"""
import json, re
...
def main():
    ...
    f_sw = open('knots_swaps_pre.jsonl', 'w')
    f_pc = open('knots_prot_claims_pre.jsonl', 'w')
    f_lc = open('knots_lp_claims_pre.jsonl', 'w')
    f_lo = open('knots_liqops_pre.jsonl', 'w')
    with open('knots_txs_pre.jsonl') as f:
        for line in f:
            d = json.loads(line)
