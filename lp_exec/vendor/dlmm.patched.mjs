// src/dlmm/index.ts
import { BN as BN24 } from "@coral-xyz/anchor";
import {
  AccountLayout,
...
var BIN_ARRAY_DEFAULT_VERSION = 3;
...
var EXTENSION_BINARRAY_BITMAP_SIZE = new BN(
  CONSTANTS.find(([_, v]) => v.name == "EXTENSION_BINARRAY_BITMAP_SIZE")?.[1].value ?? 0
);
...
var PRECISION = new Decimal("18446744073709551616");
...
var MAX_BIN_PER_LIMIT_ORDER = new BN(
  CONSTANTS.find(([_, v]) => v.name == "MAX_BIN_PER_LIMIT_ORDER")?.[1].value ?? 0
);
var LIMIT_ORDER_FEE_SHARE = new BN(
...
var FunctionType = /* @__PURE__ */ ((FunctionType2) => {
  ...
  FunctionType2[FunctionType2["LiquidityMining"] = 1] = "LiquidityMining";
  FunctionType2[FunctionType2["LimitOrder"] = 2] = "LimitOrder";
  return FunctionType2;
})(FunctionType || {});
var ConcreteFunctionType = /* @__PURE__ */ ((ConcreteFunctionType2) => {
  ConcreteFunctionType2[ConcreteFunctionType2["LimitOrder"] = 0] = "LimitOrder";
  ConcreteFunctionType2[ConcreteFunctionType2["LiquidityMining"] = 1] = "LiquidityMining";
  return ConcreteFunctionType2;
})(ConcreteFunctionType || {});
var CollectFeeMode = /* @__PURE__ */ ((CollectFeeMode2) => {
  CollectFeeMode2[CollectFeeMode2["InputOnly"] = 0] = "InputOnly";
...
var DLMMError = class extends Error {
  ...
  constructor(error) {
    ...
    if (error instanceof Error) {
      ...
      const idlError = idl_default.errors.find((err) => err.code === error);
      if (idlError) {
...
import { AnchorProvider, BN as BN20, Program as Program2 } from "@coral-xyz/anchor";
import {
...
((BinLiquidity2) => {
  function fromBin(bin, binId, binStep, baseTokenDecimal, quoteTokenDecimal, version, lbPair) {
    ...
    const pricePerToken = new Decimal2(pricePerLamport).mul(new Decimal2(10 ** (baseTokenDecimal - quoteTokenDecimal))).toString();
    ...
    if (supportLimitOrder) {
      ...
      return {
        ...
        fulfilledOrderAmountX: new BN2(0),
        fulfilledOrderAmountY: new BN2(0),
        limitOrderFeeAskSide: new BN2(0),
        limitOrderFeeBidSide: new BN2(0),
        openOrderAmount: new BN2(0),
        totalProcessingOrderAmount: new BN2(0),
        processedOrderRemainingAmount: new BN2(0),
        orderAge: 0,
...
var PositionPermission = /* @__PURE__ */ ((PositionPermission3) => {
  PositionPermission3[PositionPermission3["ClaimFee"] = 0] = "ClaimFee";
  return PositionPermission3;
})(PositionPermission || {});
var LimitOrderStatus = /* @__PURE__ */ ((LimitOrderStatus2) => {
  LimitOrderStatus2[LimitOrderStatus2["NotFilled"] = 0] = "NotFilled";
  LimitOrderStatus2[LimitOrderStatus2["PartialFilled"] = 1] = "PartialFilled";
  LimitOrderStatus2[LimitOrderStatus2["Fulfilled"] = 2] = "Fulfilled";
  return LimitOrderStatus2;
})(LimitOrderStatus || {});
...
function deriveOperator(whitelistedSigner, programId) {
  return PublicKey3.findProgramAddressSync(
...
async function getExtraAccountMetasForTransferHook(connection, mintAddress, mintAccountInfo) {
  ...
  if (!transferHook || transferHook.programId.equals(PublicKey4.default)) {
    return [];
  } else {
...
function getExcludedFeeAmount(includedFeeAmount, tradeFeeNumerator) {
  const tradingFee = includedFeeAmount.mul(tradeFeeNumerator).add(FEE_PRECISION.sub(new BN11(1))).div(FEE_PRECISION);
...
function getIncludedFeeAmount(excludedFeeAmount, tradeFeeNumerator) {
  const denominator = FEE_PRECISION.sub(tradeFeeNumerator);
  const includedFeeAmount = excludedFeeAmount.mul(FEE_PRECISION).add(denominator.sub(new BN11(1))).div(denominator);
...
function splitFee(tradingFee, protocolShare, mmAmountIn, totalAmountIn) {
  ...
  const mmFee = tradingFee.mul(mmAmountIn).add(totalAmountIn.sub(new BN11(1))).div(totalAmountIn);
  const totalLoFee = tradingFee.sub(mmFee);
  const loFee = totalLoFee.mul(LIMIT_ORDER_FEE_SHARE).div(new BN11(BASIS_POINT_MAX));
  const loProtocolFee = totalLoFee.sub(loFee);
  const mmProtocolFee = mmFee.mul(protocolShare).div(new BN11(BASIS_POINT_MAX));
...
var RebalancePosition = class {
  ...
  static async create(params) {
    ...
    let activeBin = null;
    try {
      const activeBinArrayState = await program.account.binArray.fetch(activeBinArrayPubkey);
...
function isSupportLimitOrder(lbPairState) {
  const functionType = lbPairState.parameters.functionType;
  switch (functionType) {
    case 2 /* LimitOrder */:
      return true;
    case 1 /* LiquidityMining */:
      return false;
    case 0 /* Undetermined */:
      return lbPairState.rewardInfos.every(
        (rewardInfo) => rewardInfo.mint.equals(PublicKey8.default)
      );
    default:
      throw new Error("Invalid function type");
  }
}
function getFeeMode(lbPairState, swapForY) {
  const collectFeeMode = lbPairState.parameters.collectFeeMode;
  let feeOnInput;
  let feeOnTokenX;
...
// src/dlmm/helpers/bin.ts
import BN18 from "bn.js";
function getAmountIn(amountOut, price, swapForY, rounding) {
  if (swapForY) {
...
function getAmountOut(bin, inAmount, swapForY) {
  return swapForY ? mulShr(inAmount, bin.price, SCALE_OFFSET, 1 /* Down */) : shlDiv(inAmount, bin.price, SCALE_OFFSET, 1 /* Down */);
}
function getExcludedFeeAmountIn(bin, includedFeeOutAmount, swapForY) {
  const mmAmount = swapForY ? bin.amountY : bin.amountX;
  const { openOrderAmount, processedOrderRemainingAmount } = getLimitOrderAmountsBySwapDirection(bin, swapForY);
  let remainingOutAmount = includedFeeOutAmount;
  let totalAmountIn = new BN18(0);
  let exactOutAmount = BN18.min(remainingOutAmount, mmAmount);
  let amountIn = getAmountIn(exactOutAmount, bin.price, swapForY, 0 /* Up */);
  remainingOutAmount = remainingOutAmount.sub(exactOutAmount);
  totalAmountIn = totalAmountIn.add(amountIn);
  if (remainingOutAmount.gt(new BN18(0))) {
    exactOutAmount = BN18.min(remainingOutAmount, processedOrderRemainingAmount);
    amountIn = getAmountIn(exactOutAmount, bin.price, swapForY, 0 /* Up */);
    remainingOutAmount = remainingOutAmount.sub(exactOutAmount);
    totalAmountIn = totalAmountIn.add(amountIn);
    if (remainingOutAmount.gt(new BN18(0))) {
      exactOutAmount = BN18.min(remainingOutAmount, openOrderAmount);
      amountIn = getAmountIn(exactOutAmount, bin.price, swapForY, 0 /* Up */);
      totalAmountIn = totalAmountIn.add(amountIn);
    }
  }
...
function swapExactOutQuoteAtBin(bin, binStep, sParameter, vParameter, outAmount, swapForY, supportLimitOrder, feeOnInput) {
...
function calculateExactInFillAmount(bin, amount, maxAmountOut, swapForY) {
  ...
  if (amount.gte(maxAmountIn)) {
    ...
    return {
      amountIn: amount,
      amountLeft: new BN18(0),
      outAmount
    };
  }
}
function getLimitOrderAmountsBySwapDirection(bin, swapForY) {
  ...
  return {
    openOrderAmount: new BN18(0),
    processedOrderRemainingAmount: new BN18(0)
  };
}
function getExactInFillAmountResult(bin, amountIn, swapForY, supportLimitOrder) {
  const mmAmount = swapForY ? bin.amountY : bin.amountX;
  ...
  let processedOrderAmountIn = new BN18(0);
  let processedOrderAmountOut = new BN18(0);
  let openOrderAmountIn = new BN18(0);
  let openOrderAmountOut = new BN18(0);
  ...
  const totalAmountIn = mmFillInResult.amountIn.add(processedOrderAmountIn).add(openOrderAmountIn);
  const totalAmountOut = mmFillInResult.outAmount.add(processedOrderAmountOut).add(openOrderAmountOut);
  return {
...
function swapExactInQuoteAtBin(bin, binStep, sParameter, vParameter, inAmount, swapForY, supportLimitOrder, feeOnInput) {
  let tradingFee = new BN18(0);
  let excludedFeeAmountIn = inAmount;
  const tradeFeeNumerator = getTotalFee(binStep, sParameter, vParameter);
  if (feeOnInput) {
    const { excludedFeeAmount: amount, fee: fee2 } = getExcludedFeeAmount(
      inAmount,
      tradeFeeNumerator
    );
    tradingFee = fee2;
  ...
  if (!amountLeft.isZero()) {
    ...
    if (feeOnInput) {
      const { includedFeeAmount: amount, fee: fee2 } = getIncludedFeeAmount(
        excludedFeeAmountIn,
        tradeFeeNumerator
      );
      tradingFee = fee2;
  ...
  const { fee, protocolFee } = splitFee(
    tradingFee,
    new BN18(sParameter.protocolShare),
...
function getLimitOrderLiquidity(bin, supportLimitOrder) {
  if (!supportLimitOrder) {
    return {
      orderAmountX: new BN18(0),
      orderAmountY: new BN18(0)
    };
  }
...
function getBinMaxAmountOut(bin, swapForY, supportLimitOrder) {
  const { orderAmountX, orderAmountY } = getLimitOrderLiquidity(
    bin,
    supportLimitOrder
  );
  const totalAmount = swapForY ? bin.amountY.add(orderAmountY) : bin.amountX.add(orderAmountX);
  return totalAmount;
}
...
var Observation = class {
  constructor(cumulativeActiveBinId, createdAt, lastUpdatedAt) {
    this.cumulativeActiveBinId = cumulativeActiveBinId;
...
var DynamicOracle = class {
  ...
  findCumulativeActiveIdByTimestamp(activeId, timestamp) {
    ...
    if (!earliestSample.isInitialized() || timestamp.lt(earliestSample.lastUpdatedAt)) {
      return null;
    }
    let currentIndex = this.metadata.idx.toNumber();
    while (true) {
      let previousIndex = currentIndex == 0 ? this.metadata.activeSize.toNumber() - 1 : currentIndex - 1;
      ...
      if (timestamp.gte(previousSample.lastUpdatedAt)) {
        ...
        return previousSample.cumulativeActiveBinId.mul(prevWeight).add(currentSample.cumulativeActiveBinId.mul(nextWeight)).div(totalWeight);
      }
      currentIndex = previousIndex;
    }
  }
  getActiveIdByTime(timePoint0, timePoint1) {
    const t0 = BN19.min(timePoint0, timePoint1);
    const t1 = BN19.max(timePoint0, timePoint1);
  ...
  getPriceByTime(timePoint0, timePoint1) {
...
var unwrapSOLInstruction = (owner, allowOwnerOffCurve = true) => {
...
function createProgram(connection, opt) {
  ...
  return new Program2(
    { ...idl_default, address: opt?.programId ?? LBCLMM_PROGRAM_IDS[cluster] },
    provider
  );
}
...
function getAccountDiscriminator(accountName) {
  return idl_default.accounts.find(
...
function getSlippageMaxAmount(amount, slippage) {
  ...
  const amountDecimal = new Decimal9(amount.toString());
  const slippageAppliedAmount = new BN20(
    amountDecimal.mul(new Decimal9(100 + slippage)).div(new Decimal9(100)).floor().toString()
  );
...
function getSlippageMinAmount(amount, slippage) {
  const amountDecimal = new Decimal9(amount.toString());
  return new BN20(
    amountDecimal.mul(new Decimal9(100 - slippage)).div(new Decimal9(100)).ceil().toString()
  );
}
...
function resetUninvolvedLiquidityParams(minDeltaId, maxDeltaId, favorXInActiveId, params) {
  const endBidSideDeltaId = favorXInActiveId ? new BN20(-1) : new BN20(0);
...
async function chunkDepositWithRebalanceEndpoint(dlmm, strategy, slippagePercentage, maxActiveBinSlippage, position, positionMinBinId, positionMaxBinId, liquidityStrategyParameters, owner, payer, isParallel, skipSolWrappingOperation = false) {
  ...
  for (let i = 0; i < chunkedBinRange.length; i++) {
    ...
    const minDeltaId = new BN20(chunkMinBinId - dlmm.lbPair.activeId);
    const maxDeltaId = new BN20(chunkMaxBinId - dlmm.lbPair.activeId);
    ...
    const { totalXAmount, totalYAmount } = toAmountIntoBins(
      ...
      {
        totalXAmount: new BN20(0),
        totalYAmount: new BN20(0)
      }
    );
    ...
    const rebalanceIx = await dlmm.program.methods.rebalanceLiquidity(
      {
        ...
        minWithdrawXAmount: new BN20(0),
        minWithdrawYAmount: new BN20(0),
        maxDepositXAmount,
        maxDepositYAmount,
        removes: [],
        adds: [addParam],
...
function encodePositionPermissions(permissions) {
...
var limitOrderOwnerFilter = (owner) => {
  return {
    memcmp: {
      bytes: owner.toBase58(),
      offset: 8 + 32
...
var limitOrderLbPairFilter = (lbPair) => {
  return {
    memcmp: {
      bytes: lbPair.toBase58(),
      offset: 8
...
var MAX_BPS = new BN21(BASIS_POINT_MAX);
function capBps(bps) {
  return bps.lt(new BN21(0)) ? new BN21(0) : bps.gt(MAX_BPS) ? new BN21(MAX_BPS) : bps;
}
...
var BalancedStrategyBuilder = class {
  ...
  buildRebalanceStrategyParameters() {
    ...
    let totalXAmountOut = new BN22(this.positionData.totalXAmount);
    let totalYAmountOut = new BN22(this.positionData.totalYAmount);
    totalXAmountOut = totalXAmountOut.add(new BN22(this.positionData.feeX));
    totalYAmountOut = totalYAmountOut.add(new BN22(this.positionData.feeY));
    ...
    const minDeltaId = new BN22(binPerBid).neg();
    const maxDeltaId = new BN22(binPerAsk);
    ...
    const withdrawParam = {
      minBinId: new BN22(this.positionData.lowerBinId),
      maxBinId: new BN22(this.positionData.upperBinId),
      bps: MAX_BPS
    };
    return {
      shouldClaimFee: true,
...
var LimitOrderV1Wrapper = class {
  constructor(limitOrderAddress, inner, limitOrderBinData) {
    this.limitOrderAddress = limitOrderAddress;
    this.inner = inner;
    this.limitOrderBinData = limitOrderBinData;
  }
  address() {
  ...
  getBinArrayIndexesCoverage() {
    ...
    for (const bin of this.limitOrderBinData) {
      const binArrayIndex = binIdToBinArrayIndex(new BN23(bin.binId()));
      binArrayIndexes.add(binArrayIndex.toNumber());
    }
    return [...binArrayIndexes].map((index) => new BN23(index));
  }
  getBinArrayKeysCoverage(programId) {
    return this.getBinArrayIndexesCoverage().map(
      (index) => deriveBinArray(this.lbPair(), index, programId)[0]
  ...
  parseInfo(programId, lbPair, baseMint, quoteMint, clock, binArrayMap) {
    let totalAmountXDeposited = new BN23(0);
    let totalAmountYDeposited = new BN23(0);
    let totalFilledAmountX = new BN23(0);
    let totalFilledAmountY = new BN23(0);
    let totalUnfilledAmountX = new BN23(0);
    let totalUnfilledAmountY = new BN23(0);
    let totalSwappedAmountX = new BN23(0);
    let totalSwappedAmountY = new BN23(0);
    let totalFeeAmountX = new BN23(0);
    let totalFeeAmountY = new BN23(0);
    const collectFeeMode = lbPair.parameters.collectFeeMode;
    const parsedLimitOrderBinData = [];
    const tokenXUiMultiplier = new Decimal10(10).pow(
      new Decimal10(baseMint.decimals)
    );
    const tokenYUiMultiplier = new Decimal10(10).pow(
      new Decimal10(quoteMint.decimals)
    );
    for (const loBin of this.limitOrderBinData) {
      ...
      let depositAmountX = new BN23(0);
      let depositAmountY = new BN23(0);
      let swappedAmountX = new BN23(0);
      let swappedAmountY = new BN23(0);
      let filledAmountX = new BN23(0);
      let filledAmountY = new BN23(0);
      let unFilledAmountX = new BN23(0);
      let unFilledAmountY = new BN23(0);
      let feeAmountX = new BN23(0);
      let feeAmountY = new BN23(0);
      if (loBin.isAsk()) {
      ...
      parsedLimitOrderBinData.push({
        binId: loBin.binId(),
        empty: loBin.isEmpty(),
        depositAmountX: new Decimal10(depositAmountX.toString()).div(tokenXUiMultiplier).toString(),
        depositAmountY: new Decimal10(depositAmountY.toString()).div(tokenYUiMultiplier).toString(),
        unfilledAmountX: new Decimal10(unFilledAmountX.toString()).div(tokenXUiMultiplier).toString(),
        unfilledAmountY: new Decimal10(unFilledAmountY.toString()).div(tokenYUiMultiplier).toString(),
        swappedAmountX: new Decimal10(swappedAmountX.toString()).div(tokenXUiMultiplier).toString(),
        swappedAmountY: new Decimal10(swappedAmountY.toString()).div(tokenYUiMultiplier).toString(),
        isAskSide: loBin.isAsk(),
        filledAmountX: new Decimal10(filledAmountX.toString()).div(tokenXUiMultiplier).toString(),
        filledAmountY: new Decimal10(filledAmountY.toString()).div(tokenYUiMultiplier).toString(),
        feeAmountX: new Decimal10(feeAmountX.toString()).div(tokenXUiMultiplier).toString(),
        feeAmountY: new Decimal10(feeAmountY.toString()).div(tokenYUiMultiplier).toString(),
        status: limitOrderStatus
      });
    }
    const totalXAmount = totalFeeAmountX.add(totalSwappedAmountX).add(totalUnfilledAmountX);
    const totalYAmount = totalFeeAmountY.add(totalSwappedAmountY).add(totalUnfilledAmountY);
    const transferFeeExcludedWithdrawableAmountX = calculateTransferFeeExcludedAmount(
      totalXAmount,
      baseMint,
      clock.epoch.toNumber()
    ).amount;
    const transferFeeExcludedWithdrawableAmountY = calculateTransferFeeExcludedAmount(
      totalYAmount,
      quoteMint,
      clock.epoch.toNumber()
    ).amount;
    return {
      totalDepositAmountX: new Decimal10(totalAmountXDeposited.toString()).div(tokenXUiMultiplier).toString(),
      totalDepositAmountY: new Decimal10(totalAmountYDeposited.toString()).div(tokenYUiMultiplier).toString(),
      limitOrderBinData: parsedLimitOrderBinData,
      totalUnfilledAmountX: new Decimal10(totalUnfilledAmountX.toString()).div(tokenXUiMultiplier).toString(),
      totalUnfilledAmountY: new Decimal10(totalUnfilledAmountY.toString()).div(tokenYUiMultiplier).toString(),
      totalFilledAmountX: new Decimal10(totalFilledAmountX.toString()).div(tokenXUiMultiplier).toString(),
      totalFilledAmountY: new Decimal10(totalFilledAmountY.toString()).div(tokenYUiMultiplier).toString(),
      totalSwappedAmountX: new Decimal10(totalSwappedAmountX.toString()).div(tokenXUiMultiplier).toString(),
      totalSwappedAmountY: new Decimal10(totalSwappedAmountY.toString()).div(tokenYUiMultiplier).toString(),
      totalFeeAmountX: new Decimal10(totalFeeAmountX.toString()).div(tokenXUiMultiplier).toString(),
      totalFeeAmountY: new Decimal10(totalFeeAmountY.toString()).div(tokenYUiMultiplier).toString(),
      transferFeeExcludedWithdrawableAmountX: new Decimal10(
        transferFeeExcludedWithdrawableAmountX.toString()
      ).div(tokenXUiMultiplier).toString(),
      transferFeeExcludedWithdrawableAmountY: new Decimal10(
        transferFeeExcludedWithdrawableAmountY.toString()
      ).div(tokenYUiMultiplier).toString()
    };
  }
};
function getUpdatedLimitOrderAmount(limitOrderStatus, limitOrderData, bin, collectFeeMode) {
  let fulFilledAmount = new BN23(0);
  let unFilledAmount = new BN23(0);
  switch (limitOrderStatus) {
    case 0 /* NotFilled */:
      unFilledAmount = limitOrderData.amount();
      break;
    case 1 /* PartialFilled */:
      unFilledAmount = limitOrderData.amount().mul(bin.processedOrderRemainingAmount).add(bin.totalProcessingOrderAmount.sub(new BN23(1))).div(bin.totalProcessingOrderAmount);
...
function calculateLimitOrderFee(fulfilledAmount, isAskSide, collectFeeMode, bin) {
  if (fulfilledAmount.isZero() || bin.fulfilledOrderAmountX.isZero() && isAskSide || bin.fulfilledOrderAmountY.isZero() && !isAskSide) {
    return {
      feeX: new BN23(0),
      feeY: new BN23(0)
    };
  }
  let limitOrderFee = new BN23(0);
  if (isAskSide) {
    limitOrderFee = bin.limitOrderFeeAskSide.mul(fulfilledAmount).div(bin.fulfilledOrderAmountX);
  } else {
    limitOrderFee = bin.limitOrderFeeBidSide.mul(fulfilledAmount).div(bin.fulfilledOrderAmountY);
  }
  switch (collectFeeMode) {
    case 0 /* InputOnly */:
      if (isAskSide) {
...
function getLimitOrderStatus(limitOrderBinData, bin) {
  if (limitOrderBinData.age() == bin.orderAge) {
    return 0 /* NotFilled */;
  } else if (limitOrderBinData.age() + 1 == bin.orderAge) {
    if (bin.openOrderAmount.isZero() && bin.processedOrderRemainingAmount.isZero()) {
      return 2 /* Fulfilled */;
    } else {
      return 1 /* PartialFilled */;
    }
...
function wrapLimitOrder(program, key, account) {
  const disc = account.data.subarray(0, 8);
...
// src/dlmm/helpers/limitOrders/index.ts
function decodeLimitOrderBinData(limitOrder, program, bytes) {
  const limitOrderBinData = [];
...
var DLMM = class {
   ...
   */
  static async getPairPubkeyIfExists(connection, tokenX, tokenY, binStep, baseFactor, baseFeePowerFactor, concreteFunctionType, collectFeeMode, opt) {
    const program = createProgram(connection, opt);
    ...
    presetParametersWithIndex = presetParametersWithIndex.filter((p) => {
      return p.account.concreteFunctionType == (concreteFunctionType ?? 1 /* LiquidityMining */) && p.account.collectFeeMode == (collectFeeMode ?? 0 /* InputOnly */);
    });
  ...
  static getBinIdFromPrice(price, binStep, min) {
    const binStepNum = new Decimal11(binStep).div(new Decimal11(BASIS_POINT_MAX));
    const binId = new Decimal11(price).log().dividedBy(new Decimal11(1).add(binStepNum).log());
  ...
  async getLbPairLockInfo(lockDurationOpt, getPositionsOpt) {
    ...
    const minLockReleasePoint = currentPoint.add(new BN24(lockDuration));
   ...
   * @param creatorPoolOnOffControl Whether the creator has control over enable or disable the pool.
   * @params concreteFunctionType The concrete function type of the pair, limit order or liquidity mining. Default to limit order if not provided.
   * @params collectFeeMode The collect fee mode of the pair, input only or only token Y. Default to input only if not provided.
   * @param opt An options object.
   * @returns A transaction that creates the pair.
   */
  static async createCustomizablePermissionlessLbPair2(connection, binStep, tokenX, tokenY, activeId, feeBps, activationType, hasAlphaVault, creatorKey, activationPoint, creatorPoolOnOffControl, concreteFunctionType, collectFeeMode, opt) {
    const program = createProgram(connection, opt);
    ...
    const ixData = {
      ...
      concreteFunctionType: concreteFunctionType ? concreteFunctionType : 0 /* LimitOrder */,
      collectFeeMode: collectFeeMode ? collectFeeMode : 0 /* InputOnly */,
      activationType,
   ...
   * @param creatorPoolOnOffControl Whether the creator has control over enable or disable the pool.
   * @params concreteFunctionType The concrete function type of the pair, limit order or liquidity mining. Default to limit order if not provided.
   * @params collectFeeMode The collect fee
   * @param opt An options object.
   * @returns A transaction that creates the pair.
   */
  static async createCustomizablePermissionlessLbPair(connection, binStep, tokenX, tokenY, activeId, feeBps, activationType, hasAlphaVault, creatorKey, activationPoint, creatorPoolOnOffControl, concreteFunctionType, collectFeeMode, opt) {
    const program = createProgram(connection, opt);
    const [lbPair] = deriveCustomizablePermissionlessLbPair(
      tokenX,
      tokenY,
  ...
  static async createLbPair2(connection, funder, tokenX, tokenY, presetParameter, activeId, opt) {
    ...
    const existsPool = await this.getPairPubkeyIfExists(
      ...
      tokenX,
      tokenY,
      new BN24(presetParameterState.binStep),
      new BN24(presetParameterState.baseFactor),
      new BN24(presetParameterState.baseFeePowerFactor),
  ...
  static calculateFeeInfo(baseFactor, binStep, baseFeePowerFactor) {
    ...
    const baseFeeRatePercentage = new Decimal11(baseFeeRate.toString()).mul(new Decimal11(100)).div(new Decimal11(FEE_PRECISION.toString()));
    const maxFeeRatePercentage = new Decimal11(MAX_FEE_RATE.toString()).mul(new Decimal11(100)).div(new Decimal11(FEE_PRECISION.toString()));
    return {
  ...
  getFeeInfo() {
    ...
    const protocolFeePercentage = new Decimal11(protocolShare.toString()).mul(new Decimal11(100)).div(new Decimal11(BASIS_POINT_MAX));
    return {
      baseFeeRatePercentage,
      maxFeeRatePercentage,
  ...
  getDynamicFee() {
    ...
    let activeId = new BN24(this.lbPair.activeId);
    ...
    return new Decimal11(totalFee.toString()).div(new Decimal11(FEE_PRECISION.toString())).mul(100);
  }
  /**
  ...
  getEmissionRate() {
    ...
    return {
      rewardOne: rewardOneEmissionRate ? new Decimal11(rewardOneEmissionRate.toString()).div(PRECISION) : void 0,
      rewardTwo: rewardTwoEmissionRate ? new Decimal11(rewardTwoEmissionRate.toString()).div(PRECISION) : void 0
    };
  }
  /**
  ...
  fromPricePerLamport(pricePerLamport) {
    return new Decimal11(pricePerLamport).div(
      new Decimal11(
   ...
   */
  async getLimitOrderByUserAndLbPair(userPubKey) {
    const limitOrderAccounts = await chunkedGetProgramAccounts(
      this.program.provider.connection,
      this.program.programId,
      [
    ...
    const limitOrders = limitOrderAccounts.map(
      ({ account, pubkey }) => {
        return wrapLimitOrder(this.program, pubkey, account);
      }
    );
    const binArrayPubkeysSet = /* @__PURE__ */ new Set();
    ...
    const lbPairAndBinArrays = await chunkedGetMultipleAccountInfos(
      this.program.provider.connection,
      [this.pubkey, SYSVAR_CLOCK_PUBKEY2, ...binArrayPubkeyArrays]
    );
    ...
    const parsedLimitOrder = limitOrders.map(
      (lo) => {
        const parsedLo = lo.parseInfo(
          this.program.programId,
          lbPairState,
   ...
   */
  async getLimitOrder(limitOrder) {
    const limitOrderAccount = await this.program.provider.connection.getAccountInfo(limitOrder);
  ...
  async quoteExtendPosition(currentMinBinId, currentMaxBinId, binCountToExpand) {
    ...
    const binArrayCost = new Decimal11(binArraysCount).mul(
      new Decimal11(BIN_ARRAY_FEE)
    );
    return {
      positionExtendCost: new Decimal11(positionExtendCost).div(
        new Decimal11(LAMPORTS_PER_SOL2)
      ),
      binArrayCost
    };
  }
  async quoteCreatePosition({ strategy }) {
    ...
    for (let i = 0; i < positionCount; i++) {
      ...
      const { positionExtendCost } = await this.quoteExtendPosition(
        new BN24(lowerBinId),
        new BN24(upperBinId),
        new BN24(binToExpand)
      );
      positionReallocCost += positionExtendCost.toNumber();
      lastUpperBinId = upperBinId;
    }
    const lowerBinArrayIndex = binIdToBinArrayIndex(new BN24(minBinId));
    const upperBinArrayIndex = BN24.max(
      binIdToBinArrayIndex(new BN24(maxBinId)),
      lowerBinArrayIndex.add(new BN24(1))
    );
  ...
  async quoteCreateLimitOrder({
    ...
    const resolvedBinIds = bins.map(
      (bin) => relativeBin == null ? bin.id : bin.id + this.lbPair.activeId
    );
    const { binArrayBitmapExtension, binArrayAccountMetas } = getBinArrayInfoForNonContiguousBinIds(
      resolvedBinIds,
      this.program.programId,
      this.pubkey
    );
    ...
    const limitOrderAccountSize = 8 + LIMIT_ORDER_MIN_SIZE + binCount * LIMIT_ORDER_BIN_DATA_SIZE;
    const limitOrderCost = await this.program.provider.connection.getMinimumBalanceForRentExemption(
    ...
    return {
      limitOrderCost: limitOrderCost / LAMPORTS_PER_SOL2,
  ...
  async initializeMultiplePositionAndAddLiquidityByStrategy2(positionKeypairGenerator, totalXAmount, totalYAmount, strategy, owner, payer, slippagePercentage, altAddress) {
    ...
    const liquidityStrategyParameters = buildLiquidityStrategyParameters(
      totalXAmount,
      totalYAmount,
      new BN24(minBinId - this.lbPair.activeId),
      new BN24(maxBinId - this.lbPair.activeId),
      new BN24(this.lbPair.binStep),
      strategy.singleSidedX,
      new BN24(this.lbPair.activeId),
  ...
  async initializePositionAndAddLiquidityByStrategy({
    ...
    const minBinArrayIndex = binIdToBinArrayIndex(new BN24(minBinId));
    const maxBinArrayIndex = binIdToBinArrayIndex(new BN24(maxBinId));
  ...
  async initializePositionAndAddLiquidityByWeight({
    ...
    const upperBinArrayIndex = BN24.max(
      lowerBinArrayIndex.add(new BN24(1)),
      binIdToBinArrayIndex(new BN24(upperBinId))
    );
  ...
  async addLiquidityByWeight2({
    ...
    for (const { lowerBinId: lowerBinId2, upperBinId: upperBinId2 } of chunkedBinRange) {
      const binArrayAccountsMeta = getBinArrayAccountMetasCoverage(
        new BN24(lowerBinId2),
        new BN24(upperBinId2),
        this.pubkey,
        this.program.programId
      );
      const lowerBinArrayIndex = binIdToBinArrayIndex(new BN24(lowerBinId2));
      const upperBinArrayIndex = binIdToBinArrayIndex(new BN24(upperBinId2));
      const useExtension = isOverflowDefaultBinArrayBitmap(lowerBinArrayIndex) || isOverflowDefaultBinArrayBitmap(upperBinArrayIndex);
      const binArrayBitmapExtension = useExtension ? deriveBinArrayBitmapExtension(this.pubkey, this.program.programId)[0] : null;
      const { slices, accounts: transferHookAccounts } = this.getPotentialToken2022IxDataAndAccounts(0 /* Liquidity */);
      const binCount = upperBinId2 - lowerBinId2 + 1;
      const chunkedXYAmountDistribution = xYAmountDistribution.splice(
        0,
        binCount
      );
      const chunkedQuoteDistribution = [];
      let totalChunkedXAmount = new BN24(0);
      let totalChunkedYAmount = new BN24(0);
      let totalYValue = new BN24(0);
      for (let i = 0; i < chunkedXYAmountDistribution.length; i++) {
        const distribution = chunkedXYAmountDistribution[i];
        const yAmount = totalYAmount.mul(distribution.yAmountBpsOfTotal).div(new BN24(BASIS_POINT_MAX));
        const xAmount = totalXAmount.mul(distribution.xAmountBpsOfTotal).div(new BN24(BASIS_POINT_MAX));
        const binPrice = getQPriceFromId(
          new BN24(distribution.binId),
          new BN24(this.lbPair.binStep)
        );
        const yValueOfX = totalXAmount.mul(distribution.xAmountBpsOfTotal).mul(binPrice).shrn(SCALE_OFFSET).div(new BN24(BASIS_POINT_MAX));
      ...
      const chunkedBinLiquidityDistribution = chunkedQuoteDistribution.map(
        ({ binId, yValue }) => {
          const weight = yValue.mul(new BN24(65535)).div(totalYValue);
          return {
            binId,
      ...
      preInstructions.push(...createBinArrayIxs);
      if (this.tokenX.publicKey.equals(NATIVE_MINT2) && !totalChunkedXAmount.isZero() && !this.opt?.skipSolWrappingOperation) {
        const wrapSOLIx = wrapSOLInstruction(
          user,
          userTokenX,
      ...
      if (this.tokenY.publicKey.equals(NATIVE_MINT2) && !totalChunkedYAmount.isZero() && !this.opt?.skipSolWrappingOperation) {
        const wrapSOLIx = wrapSOLInstruction(
          user,
          userTokenY,
      ...
      const addLiqIx = await this.program.methods.addLiquidityByWeight2(
        {
        ...
        tokenYProgram: this.tokenY.owner,
        binArrayBitmapExtension
      }).remainingAccounts([...transferHookAccounts, ...binArrayAccountsMeta]).instruction();
  ...
  async addLiquidityByWeight({
    ...
    const lowerBinArrayIndex = binIdToBinArrayIndex(
      new BN24(positionAccount.lowerBinId)
    );
    const [binArrayLower] = deriveBinArray(
      this.pubkey,
      lowerBinArrayIndex,
    ...
    const upperBinArrayIndex = BN24.max(
      lowerBinArrayIndex.add(new BN24(1)),
      binIdToBinArrayIndex(new BN24(positionAccount.upperBinId))
    );
    const [binArrayUpper] = deriveBinArray(
      this.pubkey,
      upperBinArrayIndex,
  ...
  async removeLiquidity({
    ...
    const binDataWithBinId = liquidityShares.map((share, i) => {
      ...
      return {
        share,
        binId: positionState.lowerBinId().add(new BN24(i)),
    ...
    for (const { lowerBinId, upperBinId } of chunkedBinRange) {
      ...
      if (shouldClaimAndClose) {
        ...
        for (let i = 0; i < 2; i++) {
          ...
          const claimRewardIx = await this.program.methods.claimReward2(new BN24(i), lowerBinId, upperBinId, {
            slices: slices2
          }).accountsPartial({
            lbPair: this.pubkey,
            sender: user,
  ...
  swapQuoteExactOut(outAmount, swapForY, allowedSlippage, binArrays, maxExtraBinArrays = 0) {
    ...
    let actualInAmount = new BN24(0);
    let feeAmount = new BN24(0);
    let protocolFeeAmount = new BN24(0);
    while (!outAmountLeft.isZero()) {
      ...
      if (!outAmountLeft.isZero()) {
        if (swapForY) {
          activeId = activeId.sub(new BN24(1));
        } else {
          activeId = activeId.add(new BN24(1));
        }
      }
    }
    ...
    const priceImpact = startPrice.sub(endPrice).abs().div(startPrice).mul(new Decimal11(100));
    actualInAmount = calculateTransferFeeIncludedAmount(
      actualInAmount,
      inMint,
      this.clock.epoch.toNumber()
    ).amount;
    const maxInAmount = actualInAmount.mul(new BN24(BASIS_POINT_MAX).add(allowedSlippage)).div(new BN24(BASIS_POINT_MAX));
    if (maxExtraBinArrays > 0 && maxExtraBinArrays <= MAX_EXTRA_BIN_ARRAYS) {
      ...
      while (extraBinArrays.length < maxExtraBinArrays) {
        ...
        if (binArrayAccountToSwapExisted) {
          ...
          if (swapForY) {
            activeId = lowerBinId.sub(new BN24(1));
          } else {
            activeId = upperBinId.add(new BN24(1));
          }
        }
      }
  ...
  swapQuote(inAmount, swapForY, allowedSlippage, binArrays, isPartialFill, maxExtraBinArrays = 0) {
    ...
    const priceImpact = new Decimal11(totalOutAmount.toString()).sub(new Decimal11(outAmountWithoutSlippage.toString())).div(new Decimal11(outAmountWithoutSlippage.toString())).mul(new Decimal11(100)).abs();
    const endPrice = getPriceOfBinByBinId(
  ...
  async seedLiquidity(owner, seedAmount, curvature, minPrice, maxPrice, base, payer, feeOwner, operator, lockReleasePoint, shouldSeedPositionOwner = false) {
    let tokenOwnerProveAssociatedTokenAccountLamports = new BN24(0);
    let totalPositionCount = new BN24(0);
    let totalPositionLamports = new BN24(0);
    let totalBinArraysCount = new BN24(0);
    let totalBinArraysLamports = new BN24(0);
    let binArrayBitmapLamports = new BN24(0);
    const toLamportMultiplier = new Decimal11(
      10 ** (this.tokenY.mint.decimals - this.tokenX.mint.decimals)
    );
    const minPricePerLamport = new Decimal11(minPrice).mul(toLamportMultiplier);
    const maxPricePerLamport = new Decimal11(maxPrice).mul(toLamportMultiplier);
    const minBinId = new BN24(
      DLMM.getBinIdFromPrice(minPricePerLamport, this.lbPair.binStep, false)
    );
    const maxBinId = new BN24(
    ...
    const decompressMultiplier = findOptimumDecompressMultiplier(
      binDepositAmount,
      new BN24(this.tokenX.mint.decimals)
    );
    ...
    const positionCount = getPositionCount(minBinId, maxBinId.sub(new BN24(1)));
    ...
    for (let i = 0; i < positionCount.toNumber(); i++) {
      const lowerBinId = minBinId.add(DEFAULT_BIN_PER_POSITION.mul(new BN24(i)));
      const upperBinId = lowerBinId.add(DEFAULT_BIN_PER_POSITION).sub(new BN24(1));
      const binArrayAccountMetas = getBinArrayAccountMetasCoverage(
        lowerBinId,
        upperBinId,
        this.pubkey,
      ...
      for (let i2 = 0; i2 < binArrayAccountMetas.length; i2++) {
        ...
        if (!account && !appendedInitBinArrayIx.has(pubkey)) {
          totalBinArraysCount = totalBinArraysCount.add(new BN24(1));
      ...
      if (!positionAccount) {
        totalPositionCount = totalPositionCount.add(new BN24(1));
      ...
      ).liquidityShares.reduce((total, cur) => total.add(cur), new BN24(0)).gt(new BN24(0));
  ...
  async seedLiquiditySingleBin(payer, base, seedAmount, price, roundingUp, positionOwner, feeOwner, operator, lockReleasePoint, shouldSeedPositionOwner = false) {
    ...
    const binId = new BN24(binIdNumber);
  ...
  async syncWithMarketPrice(marketPrice, owner) {
    ...
    const fromBinArrayIndex = binIdToBinArrayIndex(new BN24(activeBinId));
    const swapForY = marketPriceBinId < activeBinId;
    const toBinArrayIndex = findNextBinArrayIndexWithLiquidity(
      swapForY,
      new BN24(activeBinId),
    ...
    const binArrayBitMapExtensionPubkey = isOverflowDefaultBinArrayBitmap(
      new BN24(marketPriceBinArrayIndex)
  ...
  async simulateRebalancePositionWithBalancedStrategy(positionAddress, positionData, strategy, topUpAmountX, topUpAmountY, xWithdrawBps, yWithdrawBps) {
    ...
    const rebalanceStrategyBuilder = new BalancedStrategyBuilder(
      new BN24(rebalancePosition.lbPair.activeId),
      new BN24(rebalancePosition.lbPair.binStep),
      positionData,
      topUpAmountX,
      topUpAmountY,
      xWithdrawBps,
  ...
  async simulateRebalancePositionWithStrategy(rebalancePosition, rebalanceStrategy) {
    ...
    const simulationResult = await rebalancePosition.simulateRebalance(
      ...
      new BN24(this.lbPair.binStep),
      new BN24(this.tokenX.mint.decimals),
      new BN24(this.tokenY.mint.decimals),
      withdraws,
      deposits
    );
  ...
  async rebalancePosition(rebalancePositionResponse, maxActiveBinSlippage, rentPayer, slippage = 100) {
    ...
    const activeId = new BN24(lbPair.activeId);
    const { slices, accounts: transferHookAccounts } = this.getPotentialToken2022IxDataAndAccounts(0 /* Liquidity */);
    const preInstructions = [];
   ...
   */
  async enablePositionPermissionlessClaimFee({
    position,
    owner
  }) {
    const enablePositionPermissionlessClaimFeeIx = await this.program.methods.setPermissionlessOperationBits(
      encodePositionPermissions([0 /* ClaimFee */])
    ).accountsPartial({
      position,
      owner
    }).instruction();
  ...
  async placeLimitOrder({
    ...
    const resolvedBinIds = params.bins.map(
      (bin) => params.relativeBin == null ? bin.id : bin.id + this.lbPair.activeId
    );
    const { binArrayIndexes, binArrayAccountMetas, binArrayBitmapExtension } = getBinArrayInfoForNonContiguousBinIds(
      resolvedBinIds,
      this.program.programId,
      this.pubkey
    );
    ...
    const postInstructions = [];
    if (!binArrayBitmapExtension.equals(this.program.programId) && !this.binArrayBitmapExtension) {
      const initializeBitmapExtensionIx = await this.program.methods.initializeBinArrayBitmapExtension().accountsPartial({
        binArrayBitmapExtension,
        funder: owner,
        lbPair: this.pubkey,
    ...
    const [reserve, tokenMint, tokenProgram, slices, transferHookAccounts] = isAskSide ? [
      this.lbPair.reserveX,
      this.tokenX.mint.address,
      this.tokenX.owner,
      [
    ...
    const userTokenAddress = getAssociatedTokenAddressSync2(
      tokenMint,
      owner,
      true,
      tokenProgram
    );
    if (tokenMint.equals(NATIVE_MINT2) && !this.opt?.skipSolWrappingOperation) {
    ...
    const placeLimitOrderIx = await this.program.methods.placeLimitOrder(
      {
        ...params,
      ...
      payer,
      owner
    }).remainingAccounts([...transferHookAccounts, ...binArrayAccountMetas]).preInstructions(preInstructions).postInstructions(postInstructions).instruction();
    const latestBlockhashInfo = await this.program.provider.connection.getLatestBlockhash();
    const tx = new Transaction({
      ...latestBlockhashInfo,
      feePayer: sender
  ...
  async cancelLimitOrder({
    ...
    const { slices, accounts: transferHookAccounts } = this.getPotentialToken2022IxDataAndAccounts(0 /* Liquidity */);
    const { binArrayAccountMetas, binArrayBitmapExtension } = getBinArrayInfoForNonContiguousBinIds(
      binIds,
      this.program.programId,
      this.pubkey
    );
    ...
    const cancelLimitOrderIx = await this.program.methods.cancelLimitOrder(binIds, {
      slices
    }).accountsPartial({
      lbPair: this.pubkey,
      reserveX: this.lbPair.reserveX,
      ...
      tokenYProgram: this.tokenY.owner,
      memoProgram: MEMO_PROGRAM_ID
    }).remainingAccounts([...transferHookAccounts, ...binArrayAccountMetas]).instruction();
    const latestBlockhashInfo = await this.program.provider.connection.getLatestBlockhash();
    const tx = new Transaction({
      ...latestBlockhashInfo,
      feePayer: owner
  ...
  async createInitAndExtendPositionIx(lowerBinId, upperBinId, basePositionWidth, user, position) {
    ...
    if (extendedBinCount.gt(new BN24(0))) {
  ...
  static async processPosition(program, lbPair, clock, position, baseMint, quoteMint, rewardMint0, rewardMint1, binArrayMap) {
    ...
    if (!bins.length)
      return null;
    const positionData = [];
    let totalXAmount = new Decimal11(0);
    let totalYAmount = new Decimal11(0);
    ...
    bins.forEach((bin, idx) => {
      ...
      totalXAmount = totalXAmount.add(new Decimal11(positionXAmount.toString()));
      totalYAmount = totalYAmount.add(new Decimal11(positionYAmount.toString()));
      ...
      const newFeeY = posShare.isZero() ? new BN24(0) : mulShr(
        posShares[idx].shrn(SCALE_OFFSET),
      ...
      const claimableRewardsInBin = [new BN24(0), new BN24(0)];
      if (!isSupportLimitOrder(lbPair)) {
        for (let j = 0; j < claimableRewardsInBin.length; j++) {
          ...
          if (!pairRewardInfo.mint.equals(PublicKey10.default)) {
            ...
            if (bin.binId == lbPair.activeId && !bin.supply.isZero()) {
              ...
              const rewardPerTokenStoredDelta = pairRewardInfo.rewardRate.mul(delta2).div(new BN24(15)).div(liquiditySupply);
            ...
            const newReward = posShares[idx].isZero() ? new BN24(0) : mulShr(
              delta,
              posShares[idx].shrn(SCALE_OFFSET),
              SCALE_OFFSET,
              1 /* Down */
    ...
    let rewardOneExcludeTransferFee = new BN24(0);
    let rewardTwoExcludeTransferFee = new BN24(0);
    if (rewardMint0) {
    ...
    const totalXAmountExcludeTransferFee = calculateTransferFeeExcludedAmount(
      new BN24(totalXAmount.floor().toString()),
      baseMint,
      currentEpoch
    ).amount;
    const totalYAmountExcludeTransferFee = calculateTransferFeeExcludedAmount(
      new BN24(totalYAmount.floor().toString()),
      quoteMint,
      currentEpoch
    ).amount;
    return {
  ...
  static getBinsBetweenLowerAndUpperBound(lbPairKey, lbPair, lowerBinId, upperBinId, baseTokenDecimal, quoteTokenDecimal, binArrayMap, programId) {
    ...
    for (let binArrayIndex = lowerBinArrayIndex.toNumber(); binArrayIndex <= upperBinArrayIndex.toNumber(); binArrayIndex++) {
      const binArrayIndexBN = new BN24(binArrayIndex);
      const binArrayKey = deriveBinArray(
        lbPairKey,
        binArrayIndexBN,
        programId
  ...
  async getBins(lbPairPubKey, lowerBinId, upperBinId, baseTokenDecimal, quoteTokenDecimal, lowerBinArray, upperBinArray) {
    ...
    const binArrayPubkeys = range(
      ...
      (i) => deriveBinArray(lbPairPubKey, new BN24(i), this.program.programId)[0]
    );
  ...
  async createClaimBuildMethod({
    ...
    for (const {
      ...
      const binArrayAccountsMeta = getBinArrayAccountMetasCoverage(
        new BN24(chunkedLowerBinId),
        new BN24(chunkedUpperBinId),
        this.pubkey,
        this.program.programId
      );
      for (let i = 0; i < 2; i++) {
        ...
        const claimTransaction = await this.program.methods.claimReward2(new BN24(i), chunkedLowerBinId, chunkedUpperBinId, {
          slices
        }).accountsPartial({
          lbPair: this.pubkey,
          sender: owner,
  ...
  async getOracle() {
    ...
    const [oracleAccountInfo, lbPairAccountInfo] = await this.program.provider.connection.getMultipleAccountsInfo([
      oracleAddress,
      this.pubkey
    ]);
    const lbPairState = decodeAccount(
   ...
   */
  async increaseOracleLength(lengthToAdd, funder) {
    const oracleAddress = this.lbPair.oracle;
    const increaseOracleLengthIx = await this.program.methods.increaseOracleLength(lengthToAdd).accountsPartial({
