#!/usr/bin/env python3
"""H16-EARLY2 shadow (§417): 60s entry, bars re-derived from the 6 H16 opens at +60s — zero-risk OOS evidence for the mom+dd combo.
...
All results appended to h16_early2.jsonl. No orders, no wallet touches.
"""
import json, time
...
STATE = MON / "h16_early2_state.json"
LOG = MON / "h16_early2.jsonl"

SEED_LO, SEED_HI = 2.0, 10.0
ENTRY_DELAY = 60   # §415: early-window clone
HORIZON = 3600
UP, DN = 1.20, 0.92
MOM_Q67, DD_Q67, BF_Q67 = 1.0, 0.90, 0.50   # §417: native 60s bars  # §397 tercile cuts
MAX_PENDING_AGE = 300  # 60s window decides fast  # no ticks by +10m -> dead, drop
    ...
    with LOG.open("a") as lg:
        ...
        for mint in list(st["pending"]):
            ...
            if not ts:
                ...
                if age > MAX_PENDING_AGE:
                    st["pending"].pop(mint)
                    lg.write(json.dumps({"action": "h16e2_drop", "mint": mint,
            ...
            if len(pre) < 3 or not m0:
                st["pending"].pop(mint)
                lg.write(json.dumps({"action": "h16e2_thin", "mint": mint,
            ...
            if mom >= MOM_Q67 and dd >= DD_Q67 and bf >= BF_Q67:
                ...
                lg.write(json.dumps({"action": "h16e2_open", "mint": mint,
                                     "t": pre[-1][0], "scored_at": now,
                                     "entry_mcap": pre[-1][3], "mom": round(mom, 3),
                                     "dd": round(dd, 3), "bf": round(bf, 3),
                                     "seed": p["seed"]}) + "\n")
            else:
                lg.write(json.dumps({"action": "h16e2_reject", "mint": mint, "t": now,
        ...
        for mint in list(st["open"]):
            ...
            if close:
                ...
                st["open"].pop(mint)
                lg.write(json.dumps({"action": "h16e2_close", "mint": mint, "t": ct,
