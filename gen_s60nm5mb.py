...
for m, xs in tr.items():
    ...
    if ti is None or ti + 1 >= len(xs):
        continue
    if statistics.median([y.get("sol") or 0 for y in xs[:ti + 1] if y["side"] == "buy"]) < MED_MIN:
        continue
